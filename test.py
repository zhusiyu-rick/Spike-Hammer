...
def save_to_csv(eval_metrics, output_dir, filename="/neural_metrics.csv"):
    # 提取关键数据
    non_zero_data = eval_metrics["non_zero"]
    firing_rate_data = eval_metrics["firing_rate"]
    ...
    with open(output_dir, 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        # 写入表头
        writer.writerow(['time_step', 'module_name',
                         'non_zero_rate', 'firing_rate'])

        # 遍历每个时间步
        for time_step in non_zero_data:
            # 获取对应时间步的数据
            nz = non_zero_data[time_step]
            fr = firing_rate_data[time_step]
...
def main():
    ...
    try:
        ...
        save_to_csv(eval_metrics, output_dir)
        if args.local_rank == 0:
